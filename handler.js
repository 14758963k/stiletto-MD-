...
If intrested contact the owner : @${global.owner[0]} and make an order`, m);
      ...
      if (
        ...
        await conn.reply(
          m.chat,
          "*[ System Notice! ]*-Attempted contact will result in immediate termination (consider that a block). Persistent calls will be met with… *corrective measures.* Choose wisely.",
          null,
        );
      ...
      for (let name in global.plugins) {
        ...
        if (opts && match && m) {
          ...
          if (!_user.limit > 1000 && _user.limit < 1) {
            let limit = `*[SYSTEM NOTIF: YOUR LIMIT HAS EXPIRED ]*\n> • _Ding! Ding! Ding! Time's up buttercup, your free trail of exsistence expired. upgrade to premium for unlimited limit_`;
            conn.sendMessage(
              m.chat,
              {
                text: limit,
          ...
          if (plugin.level > _user.level) {
            let level = `*[ THE LEVEL IS NOT ENOUGH ]*\n> • _you so weak to play you need level *[ ${plugin.level} ]*, to Access this please play minigames or rpg to increase level_`;
            conn.sendMessage(
              m.chat,
              {
                text: level,
...
`.trim(),
                ...
                m.reply("*[ system notice ]* error occured on bot !");
              }
              m.reply(e);
            }
          } finally {
        ...
        message: {
            ...
            interactiveMessage: proto.Message.InteractiveMessage.create({
                contextInfo: {
                    ...
                    newsletterName: "Powered By Stiletto🐉",
                        serverMessageId: -1
                    }, 
                    externalAdReply: {
                        title: action === "add"
                          ...
                          : "\n──────< ʟ ᴇ ᴀ ᴠ ɪ ɴ ɢ >──────\n",
                        body: action === "add"
                          ? `👋Welcome To ${nama} from ${gpname} at ${moment.tz("Nairobi/Kenya").format("HH:mm:ss")}`
                          : `👋 Wagwan ${nama} leaving from ${gpname} at ${moment.tz("Nairobi/Kenya").format("HH:mm:ss")}`,
                        thumbnailUrl: pp,
...
│ *• Member :* ${member}
│ *• Join time :* ${moment.tz("Nairobi/Kenya").format("HH:mm:ss")}
└───────────────⭓
> [SYSTEM NOTICE]
Compliance Required:
- Rules acknowledged
...
│ *• User tag :* @${user.split('@')[0]}
│ *• Member :* ${member}
│ *• Leave time :* ${moment.tz("Nairobi/Kenya").format("HH:mm:ss")}
└───────────────⭓
