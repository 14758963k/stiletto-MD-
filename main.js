(async () => {
    require("./config");
    const { Lib, Store } = require("Stiletto-scrape");
    const {
...
global.loadDatabase = async() => {
 ...
 loadDatabase();
  global.store = makeInMemoryStore({
   ...
   const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: 'stiletto' });
      logger.level = 'fatal';
...
createTmpFolder();
   ...
   font: 'tiny',
   align: 'center',
}), CFonts.say('Powered By Stiletto', {
   colors: ['system'],
   font: 'console',
   align: 'center',
});
...
- Uptime: ${os.uptime()} seconds`));
        console.log(chalk.red.bold("[ ! ]") + chalk.cyan.bold(` Please enter your WhatsApp number, for example +254xxxx\n`) );
    ...
    reloadHandler = function(restatConn) {
        ...
        conn.welcome =
            "Welcome to *@subject* @user\nHope you enjoy the group. Dont forget to read the description\n@desc";
        conn.bye = "Goodbye @user,\naint nobody following you bruv F-off!!.";
        conn.spromote = "@user dang! just got promoted huh! Dont mess up though, am watching🫵!";
        conn.sdemote = "@user oooh maaah Gaaad🤦. lil bro just got demoted🗿";
    ...
    const watcher = chokidar.watch(path.resolve("./plugins"), {
        ...
        watcher
         ...
         if (require.cache[filename] && require.cache[filename].id === filename) {
          ...
          console.log(chalk.yellow.bold("[ Delete ] Success Delete : "+ filename.replace(process.cwd(), "")))
